...
//	}
	...
	protected void createConnector(Widget widget, AbsolutePanel panel) {
		...
		if (proxy.getText().equals(SIMPLENODE)) {
			...
			sendingService.getServiceInfo(service_name, new AsyncCallback() {
				...
				public void onFailure(Throwable caught) {
					...
					dialogBox.setText("Remote Procedure Call - Failure to get service info");
				...
				public void onSuccess(Object result) {
					...
					dialogBox.setText("Remote Procedure Call-Successful");
					serverResponseLabel.setHTML("Successfull");
					dialogBox.center();
				}
			});
		}
		...
		// 将坐标信息保存到节点内部
		l.setPixel_x(proxy.getAbsoluteLeft());
		l.setPixel_y(proxy.getAbsoluteTop());
			...
			for (int index = 0; index < node_count; index++) {
				...
				if (client_Node.getName().equals(LOOPNODE)) {
					...
					panel.add(loop, client_Loop.getX()
							- panel.getAbsoluteLeft(), client_Loop.getY()
							- panel.getAbsoluteTop());
					...
					panel.add(choice, client_Choice.getX()
							- panel.getAbsoluteLeft(), client_Choice.getY()
							- panel.getAbsoluteTop());
					...
					panel.add(fixedFor, client_FixedNumFor.getX()
							- panel.getAbsoluteLeft(), client_FixedNumFor
							.getY()
							- panel.getAbsoluteTop());
					...
					panel.add(nonFixedFor, client_NonFixedNumFor.getX()
							- panel.getAbsoluteLeft(), client_NonFixedNumFor
							.getY()
							- panel.getAbsoluteTop());
					...
					panel.add(service, client_ServiceTask.getX()
							- panel.getAbsoluteLeft(), client_ServiceTask
							.getY()
							- panel.getAbsoluteTop());
