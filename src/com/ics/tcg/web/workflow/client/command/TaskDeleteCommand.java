...
public class TaskDeleteCommand implements Command {
	...
	List<SimpleTask> backTaskList = new ArrayList<SimpleTask>();

	public TaskDeleteCommand(Widget w, DiagramBuilderExample d) {
...
//		Get_backTask_all();
//		for (Iterator iterator = backTaskList.iterator(); iterator.hasNext();) {
//			SimpleTask backSimpleTask = (SimpleTask)iterator.next();
//			for (Iterator iterator2 = backSimpleTask.webservice.iterator(); iterator2.hasNext();) {
...
//		}
		...
		for(Iterator<SimpleTask> iterator = deletewidget.backTaskList.iterator();iterator.hasNext();){
			SimpleTask simpleTask = iterator.next();
			//如果已经完成了服务配置，才能进行进一步判断
			if(simpleTask.hasFinishedConfigure){
				if(simpleTask.getText().equals("Choice")){
					ChoiceTask choice = (ChoiceTask)simpleTask;
					if(choice.check_ParamInfoWeatherComefrom(deletewidget, choice.contentPath)){
						choice.setHasFinishedConfigure(false);
						System.out.println(simpleTask.getTitle()+"的配置信息有一部分来自于"+deletewidget.getTitle());
					}
				}
				else{
				InputInfo inputInfo = simpleTask.getServiceConfigure().getMethodInfo().getInputInfo(); 
				for(Iterator<ParamInfo> iterator2 = inputInfo.getParamInfosArray().iterator();iterator2.hasNext();){
					if(simpleTask.check_ParamInfoWeatherComefrom(deletewidget, iterator2.next())){
						simpleTask.setHasFinishedConfigure(false);
						System.out.println(simpleTask.getTitle()+"的配置信息有一部分来自于"+deletewidget.getTitle());
	}
					}
				}
	...
	public void Get_backTask_all() {
		...
		for (Iterator iterator = list.iterator(); iterator.hasNext();) {
			...
			for (Iterator iterator2 = collection.iterator(); iterator2.hasNext();) {
				...
				SimpleTask front_end_simpletask = (SimpleTask) back_end_panelPanel.getParent().getParent();
