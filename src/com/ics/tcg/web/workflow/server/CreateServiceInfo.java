...
/**
 * Package the service with the type {@link ServiceInfo}.
 * Service name and methods are packaged. Input and output 
 * also packaged in the method.
...
public class CreateServiceInfo implements Serializable {
	...
	/**
	 * Create the nested paraminfos that is used.
	 * Actually this is usually called because of the parameter and 
	 * the return type defined in the javadescription package.
	 * @param c
	 * @param paramInfo
	 */
				...
				if(type.getSimpleName().equals("String")) {
					if(f.getName().equals("weatherState")) {
						...
						tempParamInfo.setParamEnums("Strong,Weak");
							...
							&& c.getSimpleName().equals("BookTrainTicketResponseInfo")) {
						tempParamInfo.setParamEnums("Seat,Sleeper,Standing");
					}
					else if(f.getName().equals("ticketKind") 
							&& c.getSimpleName().equals("BookAirTicketResponseInfo")) {
...
//					}
			...
			// not primitive
				...
				if (o instanceof ArrayList) {
					...
					String innerFUllType = name.substring(name.indexOf('<') + 1, name.indexOf('>'));
					...
					//String innerType = innerFUllType.substring(innerFUllType.lastIndexOf('.') + 1);
