...
public class QosRequirementFrameworkXmlParse implements Serializable
{
	...
	public QosRequirementFrameworkXmlParse(String xmlFileName, String serviceName)
	{
		this.xmlFileName = xmlFileName;
	...
	public QosRequirementFrameworkXmlParse(String xmlFileName, UIContent interfaceContent)
	{
		this.xmlFileName = xmlFileName;
		serviceName = new String();
	...
	public QosRequirementFrameworkXmlParse(String xmlFileName, String serviceName, UIContent interfaceContent)
	{
		this.xmlFileName = xmlFileName;
		this.serviceName = serviceName;
		this.uiContent = interfaceContent;
	...
	public void createUIContent(Element rootElement)
	{
		...
		for (int index = 0; index < nodeList.getLength(); index++)
		{
			if (nodeList.item(index) instanceof Element)
			{
				...
				createContentBlock((Element)(nodeList.item(index)), contentBlock);
	...
	public void createContentBlock(Element contentBlockElement, ContentBlock contentBlock)
	{
		...
		else
		{
			...
			for (int index = 0; index < nodeList.getLength(); index++)
			{
				...
				{
					ContentItem contentItem = new ContentItem();
					createContentItem((Element)(nodeList.item(index)), contentItem);
					contentItemList.add(contentItem);
				}
			}
		}
	...
	public void createContentItem(Element contentItemElement, ContentItem contentItem)
	{
		...
		else if (nodeList.getLength() > 1)
		{
			...
			for (int index = 0; index < nodeList.getLength(); index++)
			{
				if (nodeList.item(index) instanceof Element)
				{
					...
					if (((Element)childElement.getParentNode()).getTagName().equals("Security"))
					{
